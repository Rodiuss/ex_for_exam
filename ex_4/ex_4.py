...
gruzauto = Checkbutton(root, text="Грузовой автомобиль",
                       variable=check3, command=gruzautok)
...
gazel = Checkbutton(root, text="Газель (только малогабаритный груз)",
                    variable=check4, command=gazelk)
...
def Sum(*args):
    if float(Weight.get()) <= 0 or float(Length.get()) <= 0 or float(Width.get()) <= 0:
        showerror(title="Ошибка!",
                  message="Не пишите отрицательные числа или ноль!")
